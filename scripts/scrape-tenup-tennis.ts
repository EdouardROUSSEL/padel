...
const DATA_DIR = path.join(process.cwd(), "data", "tenup-tennis");
...
async function fetchClubs(lat: number, lng: number): Promise<TenUpResult[]> {
  const payload = {
    ...
    date: getSearchDate(),
    pratiques: ["TENNIS"],  // Changed from PADEL to TENNIS
  };
...
async function main() {
  console.log(`Starting TenUp TENNIS scraping: ${FULL_GRID.length} points`);
  ...
  console.log("");
  console.log(`Done! Total TENNIS clubs found: ${clubs.size}`);
}
